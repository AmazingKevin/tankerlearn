...
bool Score::init()
{
    ...
    scoreLabel->setPosition(Vec2(384,425));
    this->addChild(scoreLabel);
    
    int levelId = DataM::getInstance()->getLevelId();
    auto idLabel = LabelAtlas::create(String::createWithFormat("%d",levelId)->getCString(),"num1.plist");
    idLabel->setColor(Color3B(255,160,0));
    idLabel->setAnchorPoint(Vec2(1.0f,0.5f));
    idLabel->setPosition(Vec2(384,425));
    this->addChild(idLabel);
    //memcpy(_killEnemyNum, GameScene, <#size_t __n#>)
...
void Score::count(float dt)
{
    if(_step<4)
    {
        if(_countNum == 0 )
        {
            ...
            _numLabel->setPosition(Vec2(112,281 - _step*48));
